...
class Livro extends ClassePai {
    ...
    public function alterar($conn){
        $SQL = "UPDATE livros SET 
            ...
            ISSN = '$this->ISSN'
        WHERE id = $this->id";
        $conn->query($SQL); //conn possui um objeto do db, a função query executa um código sql
    }
    ...
    static public function listar($filtroNome, $conn) {
        ...
        $resultado = $conn->query($SQL);
        $retorno = [];
        while($dados = $resultado->fetch_array()){ //enquanto ele puder atribuir a dados um array dos resultados da execução do sql
            $livro = new Livro(
                $dados['id'],
                $dados['titulo'],
                $dados['autor'],
            ...
            array_push($retorno, $livro); //cria um array de livros
        }
        return $retorno;
    }
